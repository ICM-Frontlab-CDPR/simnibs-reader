"""Tests for EField, ROIExtractor, ROI, stats, and labels."""
...
class TestEField:
    ...
    def test_simulation_stored(self, efield_nii: Path) -> None:
        sentinel = object()
        acc = EField(efield_nii, simulation=sentinel)  # type: ignore[arg-type]
...
# ===========================================================================
# ROI extraction via EField.get_roi
# ===========================================================================
...
class TestGetROIFromMask:
    ...
    def test_no_source_raises(self, efield_nii: Path) -> None:
        with pytest.raises(ValueError, match="Provide exactly one"):
            EField(efield_nii).get_roi()


class TestGetROIFromSphere:
    def test_sphere_returns_values(self, efield_nii: Path) -> None:
        roi = EField(efield_nii).get_roi(coords=[5.0, 5.0, 5.0], radius=3.0)
        assert roi.n_voxels > 0
    ...
    def test_large_radius_covers_whole_volume(self, efield_nii: Path) -> None:
        roi_small = EField(efield_nii).get_roi(coords=[5.0, 5.0, 5.0], radius=1.0)
        roi_large = EField(efield_nii).get_roi(coords=[5.0, 5.0, 5.0], radius=50.0)
